...
class FITSImage(object):
   ...
   def __init__(self, filename):
      ...
      if update:
         ...
         self.header = h[0].header
      self.wcs = wcs.WCS(header=self.header, fobj=h)
   ...
   def blocksum(self, blksize):
      ...
      crpix1 = (self.header['crpix1'] + 0.5) / float(blksize) - 0.5
      crpix2 = (self.header['crpix2'] + 0.5) / float(blksize) - 0.5
      print "(crpix1, crpix2) = ", crpix1, crpix2

      # Update crpix1, crpix2
      self.header['crpix1'] = crpix1
      self.header['crpix2'] = crpix2
   ...
   def writeto(self, name=None, overwrite=False):
      ...
      if name == None:
         ...
         h = fits.open(self.filename, mode='update')
         h[0].data = self.data
         ...
         if os.path.exists(name):
            if (overwrite==False):
               ...
               os.remove(name)
         fits.append(name, self.data, self.header)
...
class WeightImage(FITSImage):
   ...
   def make_rms(self, rmsimage, cval=1.e10, norm=False):
      ...
      if os.path.exists(rmsimage):
         os.remove(rmsimage)
      fits.append(rmsimage, rms, self.header)
   ...
   def make_flg(self, flgimage, flgvalue=1):
      ...
      if os.path.exists(flgimage):
         os.remove(flgimage)
      fits.append(flgimage, flg, self.header)


def rms2wht(rms_image, output_image):
    """
    Given an image that contains the RMS values, make a weight image.
    """
    os.system('cp %s %s' % (rms_image, output_image))    
    h = fits.open(output_image, mode='update')
    rms = h[0].data
    wht = np.where(rms > 0, 1. / rms**2, 0.)
    h[0].data = wht
    h.flush()
    h.close()


def wht2flg(wht_image, output_image):
    """
    Given a weight image, make a flag image.
    """
    os.system('cp %s %s' % (wht_image, output_image))
    h = fits.open(output_image, mode='update')
    wht = h[0].data
    flg = np.where(wht > 0, 0, 1).astype('int16')
    h[0].data = flg
    h.flush()
    h.close()
