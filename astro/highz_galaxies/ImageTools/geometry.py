...
class AlignImage(object):
    def __init__(self, input_image, ref_image, input_rms, ref_rms, input_flg, ref_flg, coofile, relscale):
        ### Initialization
        ...
        # initialize WCS info
        self.hdr_ref = fits.getheader(self.ref_image)
        self.wcs_ref = wcs.WCS(self.hdr_ref)
        if 'CD1_1' in self.hdr_ref.keys():
            cd1_1 = self.hdr_ref['CD1_1']
            if 'CD1_2' in self.hdr_ref.keys():
        ...
        self.hdr_in = fits.getheader(self.input_image)
        cd1_1 = self.hdr_in['CD1_1']
        if 'cd1_2' in self.hdr_in.keys():
            cd1_2 = self.hdr_in['CD1_2']
        else:
            cd1_2 = 0.
        self.input_pixscale = np.sqrt(cd1_1**2 + cd1_2**2) * 3600.
        self.wcs_in = wcs.WCS(self.hdr_in)
        # Specify a file name to store coordinates of alignment sources
        self.coofile = coofile
        # Also pick a file name for the database output from iraf.geomap
        self.database = os.path.splitext(coofile)[0] + '.db'
    ...
    def run_sextractor(self, sexconf='', magzero=25.0, detect_thresh=10., sexcmd='cex'):
        ...
        if len(sexconf):
            ...
            sexcmd_input = [sexcmd, self.input_image]
    ...
    def get_xy_input(self, numbers):
        ...
        c = sexdf.sexdf(self.input_cat, index_col='number')
        self.x_input = np.zeros(N)
        self.y_input = np.zeros(N)
        for i in range(len(numbers)):
            self.x_input[i] = c.x_image.loc[numbers[i]]
            self.y_input[i] = c.y_image.loc[numbers[i]]
    ...
    def get_xy_ref(self, numbers):
        ...
        c = sexdf.sexdf(self.ref_cat, index_col='number')
        self.x_ref = np.zeros(N)
        self.y_ref = np.zeros(N)
        for i in range(len(numbers)):
            self.x_ref[i] = c.x_image.loc[numbers[i]]
            self.y_ref[i] = c.y_image.loc[numbers[i]]

    def write_xy_map(self):
    ...
    def read_shifts(self):
        ...
        ra0, dec0 = self.wcs_in.wcs_pix2world([[1,1]], 1)[0]
        x0ref_old, y0ref_old = self.wcs_ref.wcs_world2pix([[ra0, dec0]], 1)[0]
    ...
    def apply_shift(self, input_image="", suffix='geo', force_north_up=True):
        ...
        xshift, yshift, xrot = self.read_shifts()
    ...
    def run_alignment(self, reflist, suffix='geo', fitgeometry='rscale'):
        """
        Align the images after running SExtractor.
        reflist: the name of a file that contains the SExtractor IDs of the
                 bright sources for alignment. There should be two columns,
                 one for the IDs from the INPUT image and one for the the IDs
        ...
        r = sexdf.sexdf(reflist)
        self.get_xy_input(r.segnum_in.values)
        self.get_xy_ref(r.segnum_ref.values)
        self.write_xy_map()
        self.geomap(fitgeometry=fitgeometry)
        # self.geotran()
        # self.apply_shift(suffix=suffix)

    def calculate_angular_shift(self, shifts=[]):
        """
        Convert the geomap'd xshift, yshift into angular units.
        """
        if len(shifts) == 0:
        ...
        # calculate the old origin of the reference image
        radec0 = self.wcs_ref.wcs_pix2world([[1, 1]], 1)
        x0, y0 = self.wcs_in.wcs_world2pix(radec0, 1)[0]
        xshift_deg = (x0 - xshift) * self.input_pixscale / 3600.
        yshift_deg = (y0 - yshift) * self.input_pixscale / 3600.
        return xshift_deg, yshift_deg

    def calculate_new_crval(self, output_image, shifts=[]):
        """
        Using the angular shift from calculate_angular_shift to calculate the
        new CRVALs to be used for the input image.
        """
        xshift, yshift = self.calculate_angular_shift(shifts=shifts)
        crval1_new = self.hdr_in['crval1'] - xshift
        crval2_new = self.hdr_in['crval2'] - yshift
        # update output image
        os.system('cp {} {}'.format(self.input_image, output_image))
        h = fits.open(output_image, mode='update')
        h[0].header['crval1'] = crval1_new
        h[0].header['crval2'] = crval2_new
        h.flush()
        h.close()

        return crval1_new, crval2_new
...
def paste_image(source, dest, output):
    ...
    # source image
    ra0, dec0 = s.wcs.wcs_pix2world([[1, 1]], 1)[0]
    x0d, y0d = d.wcs.wcs_world2pix([[ra0, dec0]], 1)[0]
    ...
    newwcs = wcs.WCS(d.header)
    newcrpix1 = d.header['crpix1'] - dx
    newcrpix2 = d.header['crpix2'] - dy
    newcrval1, newcrval2 = newwcs.wcs_pix2world([[newcrpix1, newcrpix2]], 1)[0]
    ...
    fits.writeto(output, newdata, header=newheader, clobber=True)
