...
def merge_galfit(outputfile, galfit_output_dir, galfit_input_dir, band):
   # If have not merged everything yet... merge now
   # this will be a SExtractor-format catalog, with FILENAME as the first
   # column.
   rh.write_gf_output(galfit_output_dir,'obj*out.fits', 'temp.cat')
   
   # Add the objects that GALFIT crashed on
   add_crashes('temp.cat', galfit_input_dir)

   # Add a column that has the object SExtractor IDs
   add_objid('temp.cat')

   # Add a prefix to all column names showing which filter these quantities are
   # measured in
   add_header_prefix('temp.cat', band)

   # Reformat the GALFIT catalog
   reformat_gf_catalog('temp.cat', outputfile, '{}_filename'.format(band))
...
def add_crashes(outputfile, inputdir):
   ...
   Arguments:
   ------------------------------------
   outputfile:       file name of all GALFIT results; this is the output from
                     readgfheader.write_gf_output
   inputdir:         directory that has the GALFIT input files obj*[0-9]
   ------------------------------------
   """
...
def reformat_gf_catalog(galfit_catalog, galfit_catalog2, fn_col):
   """
   Re-format this and make object IDs as the first column, and then write to
   a merged galaxy catalog with GALFIT-measured values.
   """
   c = sextractor(galfit_catalog)
   objectid = N.zeros(len(c), 'int')
   filenames = getattr(c, fn_col)
   for i in range(len(c)):
      root = filenames[i].split('_')[0]
      objectid[i] = int(root[3:])
      # root = os.path.splitext(c._fname)[0]  # in the format of objXXXX, where XXXX is the object ID
      # objectid[i] = int(root[3:-4])  # the object ID
   header = c._header.replace('FILENAME', 'OBJECTID')
   # Now write to the catalog
   f = open(galfit_catalog2,'w')
   f.write(header)
   for i in range(len(c)):
