...
from surfsup.fitsutil import FitsDF
from sexdf import sexdf
...
def run_galfit(paramfile):
    ...
    *** Make sure that target_catalog and all_catalog have matching IDs! ***
    ...
    if pars['target_catalog'].endswith('.fits'):
        c = FitsDF(pars['target_catalog'], index_column=pars['id_column'])
        # c = Ftable(pars['target_catalog'])
    else:
        c = sexdf(pars['target_catalog'], index_col=pars['id_column'])
        # c = sextractor(pars['target_catalog'])
        # c.__getitem__ = c.__getattribute__
    Ncat_target = len(c)

    if pars['all_catalog'].endswith('.fits'):
        c_all = FitsDF(pars['all_catalog'], index_column=pars['id_column'])
        # c_all = Ftable(pars['all_catalog'])
    else:
        c_all = sexdf(pars['all_catalog'], index_col=pars['id_column'])
    ...
    object_id_target = c.index
    object_id_all = c_all.index
    ra = c_all[pars['ra_column']]
    dec = c_all[pars['dec_column']]
    ...
    mag = c_all[pars['mag_column']]
    Re = c_all[pars['re_column']]
    ellip = c_all[pars['ell_column']]
    axratio = 1. - ellip
    theta_sex = c_all[pars['theta_column']]
    theta = theta_sex + 90.
    # osect = acscat.osect_refnum
    isoa = c_all[pars['isoa_column']]
    ...
    if (pars['nblocks_x'] > 1) or (pars['nblocks_y'] > 1):
        # Now figure out which sub-panel is each galaxy in
        ...
        # Cut images; also include padding around each subpanel is possible
            ...
            for ny in range(pars['nblocks_y']):
                ...
                if pars['split_image']:
                    if not os.path.isdir('subpanels'):
                        os.mkdir('subpanels')
                    print "Copying subpanel (%d, %d): [%d:%d,%d:%d]" % (
                        nx+1, ny+1, x0, x1, y0, y1)
                ...
                # Store the mask for each subpanel
                mask0_dic[(nx+1, ny+1)] = pyfits.getdata(
                    'subpanels/input_seg_%d_%d.fits' % (nx+1, ny+1))
                
                # Now calculate the pixel coordinates of all objects in this
                # subpanel
                hdr_subpanel = pyfits.getheader(
                    'subpanels/input_drz_%d_%d.fits' % (nx+1, ny+1))
                wcs_subpanel = pywcs.WCS(hdr_subpanel)
                print "Calculate (x, y) in subpanel ({}, {}) from WCS...".format(
                    nx+1, ny+1)
    ...
    # Now start looping through all galaxies and measure!
    for i in range(pars['startnum'], pars['stopnum']):
        ...
        magarray = [mag.loc[objid]]
        rearray = [Re.loc[objid]]
        ...
        if (yposarray[0] < 0) | (yposarray[0] > nlines):
            print "Object %d is outside the footprint of this image." % objid
            continue
        theta_array = [theta.loc[objid]]
        axrat_array = [axratio.loc[objid]]
        segnum_array = [object_id_target[i]]
        ncomp = 1

        # step 1: determine temporary image size   
        # Experimental image size
        majx = abs(np.sqrt(isoa.loc[objid]) * 4. * np.sin(
                   theta.loc[objid] / 180. * np.pi))
        # convert degree into radian
        majy = abs(np.sqrt(isoa.loc[objid]) * 4. * np.cos(
                   theta.loc[objid] / 180. * np.pi))
        minx = axratio.loc[objid] * np.sqrt(
            isoa.loc[objid]) * 4. * abs(
            np.sin((theta.loc[objid] + np.pi / 2.) / 180. * np.pi))
        miny = axratio.loc[objid] * np.sqrt(
            isoa.loc[objid]) * 4. * abs(
            np.sin((theta.loc[objid] + np.pi / 2.) / 180. * np.pi))
        Re_max = 20.
        if Re.loc[objid] > Re_max:
            Re_max = Re.loc[objid]
        ...
        for j in range(Ncat):  # iterate through other objects
            if object_id_all[j] != objid:
                ...
                radck = np.sqrt(isoa.loc[object_id_all[j]])
                angle = np.arctan2(dy, dx)
                phi = np.pi/2. - (
                    theta.loc[object_id_all[j]] / 180. * np.pi + np.pi/2. - angle)
                thetap = np.arctan2(
                    axratio.loc[object_id_all[j]] * np.tan(phi), 1.)
                isorad = radck * np.sqrt(
                    (axratio.loc[object_id_all[j]] * np.cos(thetap))**2 + np.sin(thetap)**2)
                ...
                if (((x_image[ixy][j] >= xmin) and (x_image[ixy][j] <= xmax) and
                  ...
                  (dist < re_factor * Re.loc[objid]))) or 
                  ...
                  mag[j] + 1. <= magarray[0])):
                    ...
                    magarray += [mag.loc[object_id_all[j]]]
                    rearray += [Re.loc[object_id_all[j]]]
                    axrat_array += [axratio.loc[object_id_all[j]]]
                    theta_array += [theta.loc[object_id_all[j]]]
                    segnum_array += [object_id_all[j]]
                    if Re[j] > Re_max:
                        Re_max = Re.loc[object_id_all[j]]
        ...
        if pars['mask'] == True:
            ...
            mask2_flat = np.where(np.in1d(mask0_flat, sidarray), 0, mask0_flat)
            ...
            mask4 = np.where(mask4 <= 10., 0, 1).astype('int')
