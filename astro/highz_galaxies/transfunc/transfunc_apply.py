...
def conv_kernel(key, kgrid, bmodel):
   ...
   x0 = np.array([kgrid.xedges()[k[0]], kgrid.yedges()[k[1]]])
   x1 = x0 + np.array([kgrid.dx, kgrid.dy])
...
def apply_transfer_2d(bmodel, kgrid, nproc_model=1):
   ...
   #bmodel.model = bmodel.model * kgrid.completeness_model
   convolved_model = zeros(modshape,"float")
