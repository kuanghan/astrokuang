...
import numpy as np
import os, sys, glob, time, string, pdb
from sextutils import sextractor
import yaml
import sexdf
import pandas as pd
from surfsup import fitsutil as fu
...
class SExtractorSim(object):
    def __init__(self, parfile):
        ...
        if 'DETECT_IMAGE' in self.c.keys():
            self.detect_image = self.c['DETECT_IMAGE']
        else:
        ...
        hdr = fits.getheader(self.detect_image)
        ...
        fg_kwargs = dict(ngal=self.ngal, diskfrac=self.diskfrac, 
                    ...
                    mag0=self.maglow, mag1=self.maghigh, logr0=self.logrmin,
                    logr1=self.logrmax, 
        ...
        if 'POSFILE' in c.keys():
            self.posfile = c['POSFILE']
   
    def makenoiselessimage(self, band, galfile, magz, psffile, xmax=-1, ymax=-1, save=0, gain=1.0):
        """
        Creates a noiseless convolved image
        """
        assert os.path.exists(psffile), "PSF image %s does not exist." % psffile
        assert os.path.exists(galfile), "Fake galfile {} does not exist.".format(galfile)
        
        print "galfile =", galfile
        broot = self.root + '_%s' % band
        outfile = broot + '_sim.fits'
        outfile_nonoise = broot + '_sim_noiseless.fits'
        
        # Determine the size of the noiseless image
        if xmax < 0:
            xmax = self.xmax
        if ymax < 0:
            ymax = self.ymax
        ...
        print "Using magz = {:.2f} & gain = {:.2f}".format(magz, gain)
    ...
    def addsimulated(self, band, save=0):
        ...
        noiseless_img = fits.getdata(self.noiselessimages[band])
        realimage_img = fits.getdata(self.realimages[band])
        print "Noiseless image: {} (shape={})".format(
            self.noiselessimages[band], noiseless_img.shape)
        print "Real image: {} (shape={})".format(
            self.realimages[band], realimage_img.shape)

        hdr = fits.getheader(self.realimages[band])
        ...
        if self.psfmatch:
            if band == self.detect_band:
                ...
                kernel = fits.getdata(self.psfmatch_kernels[band])
        ...
        fits.append(outimage, simulated_img, hdr)
    ...
    def insert_fake_sources(self):
        ...
        if hasattr(self, 'posfile'):
            print("Generate random positions from file {}".format(self.posfile))
            fg_args = [self.realimages, self.flagimages, self.bands,
                self.posfile]
            fg_kwargs = dict(ngal=self.ngal, diskfrac=self.diskfrac, 
                        magfile=self.magfile, rdist=self.rdistfunc, 
                        ...
                        flagmax=self.flagmax,
                        othercols_list=self.othercolnames)
            self.fg = fake_galaxies.FakeGalaxiesCustomXY(*fg_args, **fg_kwargs)
        else:
            # Generate random positions for fake sources
    ...
    def collect_results(self, output_name, colname_file=None, booleancols=['detect']):
        """
        Collect all simulation results into one master catalog.
        """
        bandcat = {}
        ...
        # sexpars = open(self.sextparfile).readlines()
        # sexcols = []
        # for line in sexpars:
        #    line = line.strip()
        #    if len(line) == 0 or line[0] == '#':
        #       continue
        #    sexcols.append(line.split()[0])
        # cname['SEXTRACTOR_COLUMNS'] = sexcols
        ...
        for b in self.bands:
            broot = '%s_%s' % (self.root, b)
            cb = sexdf.sexdf('%s/%s' % (broot, bandcat[b]))
            if b == self.detect_band:
                cb0 = cb
            for cn in cname['SEXTRACTOR_COLUMNS']:
                name = '%s_%s' % (b.lower(), cn.lower())
                if cn.lower() in cb.columns:
                    df[name] = cb[cn.lower()]
                    colnames += [name]
            ...
            if b == self.detect_band:
                colnames += ['detect']
                df['detect'] = np.where(cb['x_image'] > 0, 1, 0)
        ...
        for cn in cname['INPUT_ALL_COLUMNS'] + self.othercolnames:
            name = cn.upper()
            df[name] = cb0[cn.lower()]
            colnames += [name]

        # Now write to output catalog... if the file extension is ".fits", write
        # to a FITS table. Otherwise, write to an ASCII file in the SExtractor
        # format.
        if os.path.splitext(output_name)[-1] == '.fits':
            # df = pd.DataFrame(data=columns)
            fu.df2fits(df, output_name, booleancols=booleancols)
        else:
            # write headers
            sexdf.write_cat(df, output_name)
            # print colnames
            # f = open(output_name, 'w')
            # for i in range(len(colnames)):
            #     f.write('# {} {}\n'.format(i+1, colnames[i]))
