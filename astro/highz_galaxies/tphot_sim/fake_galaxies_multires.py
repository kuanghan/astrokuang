...
class FakeGalaxiesMultiRes(fg.FakeGalaxies):
   ...
   def makegals_multiband(self, flagimage, igalfile=""):
      """
      Make artdata object list for the high-res image only.
      """
   ...
   def makegals_lores(self):
      ...
      Note: we only insert point sources into the low-res image, assuming that
      all sources are not resolved in the low-res image.
      """
      ...
      print self.mag[self.bands[1]]
      
      # input file for iraf.mkobject
      f = open(self.artfiles[self.bands[1]], "w")
      for i in range(self.ngal):
         # Write lines for the mkobjects galaxy list
         # Insert PSF
         f.write("{:.2f} {:.2f} {:.2f}".format(
                 self.x_lores[i], self.y_lores[i], self.mag[self.bands[1]][i]
                 )
         )
